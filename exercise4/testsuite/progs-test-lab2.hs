...
testGoodProgram prog f =
    do input  <- readFileIfExists (f++".input")
       ...
       debug $ "Exit code: " ++ show s
       case lines out of
           "TYPE ERROR":_ -> do reportError c "Passed good program" f "" out err
                                return False
           "SYNTAX ERROR":_ -> do reportError c "Passed good program" f "" out err
                                  return False
           _ -> return True
...
readFileIfExists f = catch (readFile f) (\(e :: IOException) -> return "")
